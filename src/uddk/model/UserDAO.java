...
public class UserDAO {
    ...
    public static boolean login(String username, String password) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "SELECT COUNT(Magiaovien) FROM thongtinnguoidung WHERE username = ? AND password = ?";
    ...
    public static boolean isAdmin(String username) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "SELECT COUNT(Magiaovien) FROM thongtinnguoidung WHERE username = ? AND role = 1";
            connection = dbContext.getConnection();
            ps = connection.prepareStatement(sql);
            ps.setString(1, username);
            rs = ps.executeQuery();
    ...
    public static User getUserByUserId(String userId) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "SELECT * FROM thongtinnguoidung WHERE Magiaovien = ?";
            connection = dbContext.getConnection();
            ps = connection.prepareStatement(sql);
            ...
            while (rs.next()) {
                return new User(rs.getString("Magiaovien"), rs.getString("Ten"), rs.getString("Chucvu"), rs.getInt("Tuoi"), rs.getString("username"), rs.getString("password"), rs.getInt("role"));
            }
    ...
    public static int deleteUserById(String userId) throws Exception {
        ...
        int n = 0;
        try {
            String sql = "DELETE FROM thongtinnguoidung WHERE Magiaovien = ?";
            connection = dbContext.getConnection();
            ps = connection.prepareStatement(sql);
            ps.setString(1, userId);
            n = ps.executeUpdate();
